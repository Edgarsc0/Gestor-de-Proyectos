...
import { ProgressBar, AvatarStack, ProjectStatusBadge, Avatar } from "@/components/Badges";
import Modal from "@/components/Modal";
import Select from "@/components/Select";
import {
  ChevronRight, FolderOpen, CheckSquare, Users, Plus,
  Building2, Crown, ArrowRight
} from "lucide-react";
import { useSession } from "next-auth/react";

const COLORS = ["#8B1515","#A31B1B","#1D4ED8","#0F766E","#6D28D9","#B45309","#065F46","#9D174D"];
...
export default function AreaDetailPage() {
  ...
  const fetchArea = useCallback(async () => {
    ...
    if (!res.ok) { router.push("/dashboard/areas"); return; }
    const data = await res.json();
    setArea(data);
    setLoading(false);
  }, [id, router]);

  useEffect(() => { fetchArea(); }, [fetchArea]);
  ...
  const inProgressTasks = area.projects?.reduce((s, p) => s + (p.tasks?.filter(t => t.status === "IN_PROGRESS").length || 0), 0) || 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div variants={fadeUp} initial="hidden" animate="show" custom={0}
        ...
        <span className="text-slate-700 dark:text-slate-300 font-medium truncate">{area.name}</span>
      </motion.div>
      ...
      <motion.div variants={fadeUp} initial="hidden" animate="show" custom={1}
        ...
        <div className="p-6">
          <div className="flex flex-col sm:flex-row sm:items-start gap-5">
            {/* Icon */}
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-white font-display font-bold text-3xl flex-shrink-0 shadow-xl"
              style={{ backgroundColor: area.color, boxShadow: `0 8px 24px ${area.color}50` }}>
              {area.name.charAt(0)}
            </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              {area.description && (
                <p className="text-slate-500 dark:text-slate-400 text-sm mb-4">{area.description}</p>
              )}

              {/* Stats */}
              <div className="flex flex-wrap items-center gap-6 mb-4">
                <div className="text-center">
                  <p className="text-xl font-bold text-slate-900 dark:text-slate-100">{totalProjects}</p>
      ...
      <motion.div variants={fadeUp} initial="hidden" animate="show" custom={2}>
        ...
        {totalProjects === 0 ? (
          <div className="card p-12 text-center">
            ...
            <p className="font-medium text-slate-600 dark:text-slate-400">Sin proyectos en esta área</p>
            <p className="text-xs text-slate-400 mt-1">Crea el primer proyecto para comenzar</p>
          </div>
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {area.projects.map((project, i) => {
              ...
              const doneT = project.tasks?.filter(t => t.status === "COMPLETED").length || 0;

              return (
                <motion.div key={project.id} variants={fadeUp} initial="hidden" animate="show"
                  custom={i * 0.08} whileHover={{ y: -3, transition: { duration: 0.2 } }}>
                  <Link href={`/dashboard/proyectos/${project.id}`} className="block card-hover p-5 group h-full">
                    {/* Color accent */}
                    <div className="h-1 rounded-full mb-4 -mx-1" style={{ backgroundColor: project.color }} />

                    <div className="flex items-start gap-3 mb-3">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-base flex-shrink-0 shadow"
                        style={{ backgroundColor: project.color, boxShadow: `0 3px 10px ${project.color}50` }}>
                    ...
                    <div className="space-y-2 mb-3">
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-slate-500">{doneT} de {totalT} tareas</span>
      ...
      {areaMembers.length > 0 && (
        <motion.div variants={fadeUp} initial="hidden" animate="show" custom={3}>
          <h2 className="font-semibold text-slate-900 dark:text-slate-100 mb-3">
            Miembros del área
            <span className="ml-2 text-sm font-normal text-slate-400">({areaMembers.length})</span>
          </h2>
                ...
                <motion.div key={user.id} variants={fadeUp} initial="hidden" animate="show"
                  ...
                  <div className="flex flex-col items-end gap-1 flex-shrink-0">
                    ...
                    <span className="text-[11px] text-slate-400">
                      {userProjectCount} proyecto{userProjectCount !== 1 ? "s" : ""}
                    </span>
                  </div>
                </motion.div>
              );
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Nuevo proyecto">
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 p-3 rounded-xl text-sm"
            ...
            <span className="font-medium" style={{ color: area.color }}>Área: {area.name}</span>
          </div>

          <div>
            <label className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 block">Nombre del proyecto</label>
            <input className="input-field" placeholder="Ej: Rediseño del portal web"
              value={formData.name}
          ...
          <div>
            ...
            <textarea className="input-field resize-none h-20" placeholder="¿De qué trata este proyecto?"
